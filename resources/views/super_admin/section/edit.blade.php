...
<div class="panel mt-3">
    <h5 class="text-lg font-semibold dark:text-white-light">تعديل {{ $section->name }}</h5>

    <form class="mt-5" autocomplete="off" action="{{ route('super_admin.section.update') }}" method="POST">
        @csrf
        <input type="hidden" name="id" value="{{ $section->id }}">
        @include('super_admin.section.form')
