<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div>
        ...
        <input name="name" type="text" placeholder="ادخل اسم اللجنة" class="form-input" value="{{ old('name', $section->name ?? '') }}" autocomplete="off" />
        @error('name') <span class="text-danger">{{ $message }}</span> @enderror
    </div>
    <div>
        <label for="username">اسم المستخدم</label>
        <input name="username" type="text" placeholder="ادخل اسم المستخدم" class="form-input" value="{{ old('username', $section->username ?? '') }}" autocomplete="off" />
        @error('username') <span class="text-danger">{{ $message }}</span> @enderror
    </div>
    <div>
        <label for="email">البريد الالكتروني</label>
        <input name="email" type="email" placeholder="ادخل البريد الالكتروني" class="form-input" value="{{ old('email', $section->email ?? '') }}" autocomplete="off" />
