    ...
    <div class="flex items-start justify-center min-h-screen px-4">
        <div x-show="open" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                <div class="font-bold text-lg">تعديل  مشاركة</div>
            ...
            <form action="{{ route('super_admin.contribution.update') }}" method="post">
            @csrf
            <input type="hidden" name="id" value="{{$contribution->id }}">
            @include('super_admin.contribution.form')
    
            <div class="flex justify-end items-center p-5">
                <button type="button" class="btn btn-outline-danger" @click="toggle">غلق</button>
                <button type="submit" class="btn btn-primary ltr:ml-4 rtl:mr-4" wire:click="addNewVolunteer" >تعديل</button>
            </div>
            </form>
        </div>
