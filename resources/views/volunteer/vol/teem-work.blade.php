...
@section('content')
    ...
    <ul class="flex space-x-2 rtl:space-x-reverse">
        <li>
            <a href="javascript:;" class="text-primary hover:underline">فريق العمل </a>
        </li>
            ...
            <tbody>
                @foreach ($volunteers as $volunteer)
                    <tr>
                        ...
                        <td class="text-center">{{ $volunteer->section->name ?? 'لا يوجد' }}</td>
                        <td class="text-center">{{ $volunteer->position ?? 'لا يوجد' }}</td>
                        <td class="text-center">
                            @if ($volunteer->getMonthlyCountAttribute() > 0)
                            <div class="bg-success h-4 rounded-full text-center text-white flex justify-between items-center px-2 text-xs" 
                                style="width: {{ ($volunteer->getMonthlyCountAttribute() ?? 0) > 100 ? 100 : $volunteer->getMonthlyCountAttribute() }}%;">
                                <span>{{ $volunteer->getMonthlyCountAttribute() }}</span>
                                <span>{{ ($volunteer->getMonthlyCountAttribute() ) > 8 ? 100 : $volunteer->getMonthlyCountAttribute() }}%</span>
                            </div>
                            @else
                            <span class="badge bg-danger">لم يشارك بعد</span>
