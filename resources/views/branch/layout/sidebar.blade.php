    ...
    <nav x-data="sidebar" class="sidebar fixed bottom-0 top-0 z-50 h-full min-h-screen w-[260px] shadow-[5px_0_25px_0_rgba(94,92,154,0.1)] transition-all duration-300">
        <div class="h-full bg-white dark:bg-[#0e1726]">
            <div class="flex items-center justify-between px-4 py-3">
                <a href="{{ route('branch.index') }}" class="main-logo flex shrink-0 items-center">
                    <img class="ml-[5px] w-8 flex-none" src="{{ asset('assets') }}/images/logo.svg" alt="image" />
                    <span class="align-middle text-2xl font-semibold rtl:ml-1.5 rtl:mr-1.5 dark:text-white-light lg:inline">VMS</span>
                </a>
            ...
            <ul class="perfect-scrollbar relative h-[calc(100vh-80px)] space-y-0.5 overflow-y-auto overflow-x-hidden p-4 py-0 font-semibold" x-data="{ activeDropdown: 'dashboard' }">
                <li class="nav-item  ">
                    <a href="{{ route('branch.index') }}" class="{{(request()->routeIs('branch.index')) ? 'active':''}} group">
                        <div class="flex items-center">
                            ...
                            <span class="text-black rtl:pl-3 rtl:pr-3 dark:text-[#506690] dark:group-hover:text-white-dark">الرئيسية</span>
                        </div>
                    </a>
                </li>
                        ...
                        <li class="menu nav-item">
                            <button type="button" class="nav-link group" :class="{'active' : activeDropdown === 'events'}" @click="activeDropdown === 'events' ? activeDropdown = null : activeDropdown = 'events'">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-black ltr:pl-3 rtl:pr-3 dark:text-[#506690] dark:group-hover:text-white-dark">الأنشطة</span>
                                </div>
                            ...
                            <ul x-show="activeDropdown === 'events'" x-collapse="" class="sub-menu text-gray-500" style="height: 0px; overflow: hidden; display: none;" hidden="">
                                @foreach (auth('branch')->user()->activities as $activity)

                                <li>
                                    <a href="{{ route('branch.activity.events',$activity->id) }}">

                                        {{ $activity->name }}
                                        @if ($activity->getPendingEventsCountAttribute() > 0)
                                        <span class="badge badge-outline-danger ltr:ml-auto rtl:mr-auto">{{ $activity->getPendingEventsCountAttribute() }}</span>
                                        @endif
